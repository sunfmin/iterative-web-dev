...
### Step 6: Review Each Screenshot

**CRITICAL**: Use the Read tool to open and visually inspect EVERY screenshot.

#### Layout
- Does content fit without overflow?
- Any clipping or cut-off elements?
- Proper alignment?
...
#### Error States
- Error messages visible and red?
- Clearly associated with the input?
- Helpful error text?
...
test('user can login', async ({ page }) => {
  ...
  // Screenshot: Login page initial state (scope from .active-scope)
  const scope = 'auth'; // Read from .active-scope in real tests
  await page.screenshot({ 
    path: `e2e/screenshots/${scope}-feature-1-step1-login-initial.png`,
    fullPage: true 
  });
  ...
  await page.screenshot({ 
    path: `e2e/screenshots/${scope}-feature-1-step2-dashboard-after-login.png`,
    fullPage: true 
  });
});
```
...
- Capture at key user journey points
- Include error states
- Capture responsive breakpoints if needed
...
- `auth-feature-11-cancel-keeps-project.png`
